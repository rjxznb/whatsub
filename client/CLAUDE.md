...
- **JSON Lines streaming for LLM output.** The model returns only generated cue fields (`index`, `translation`, compact `highlights`); source text and timestamps always come from the local SRT. Every validated cue is published immediately as an in-memory preview, while malformed/missing/empty cues are repaired individually with prompts containing only unresolved inputs. Invalid optional highlights are dropped without discarding a valid translation. DeepSeek analysis gets four bounded attempts; malformed summary output retries the summary only.
