...
- **JSON Lines streaming for LLM output.** The canonical provider output is one compact `{i,zh,p}` object per cue, where `p` contains zero to two `[source, translation, note]` tuples and `isKeyPoint` is derived locally. Source text and timestamps always come from the local SRT. The parser temporarily accepts the older verbose `index/translation/highlights` and `highlightWords/keyNotes/highlightTranslations` shapes as input-only compatibility paths. Learning expressions should contain one to five English words and can never exceed eight; a long whole-cue selection is rejected locally. Every validated cue is published immediately as an in-memory preview, while malformed/missing/empty translations are repaired individually with prompts containing only unresolved inputs. If translation is valid but all claimed annotations are malformed, the accepted translation is preserved and only that cue's annotations receive one targeted repair round; another malformed annotation response degrades to translation-only instead of replaying the batch. The inflight entry persists `annotationRepair:true` until that round is resolved, and journal monotonicity permits exactly one annotation-only upgrade with immutable cue identity and translation, so process restart resumes the repair without allowing arbitrary subtitle rewrites. All providers use four bounded transport/content attempts for ordinary cue and summary analysis.
- **Analysis persistence = runtime lease + canonical checkpoint + bounded inflight journal.** `openStoredAnalysisSession` reads `transcript.srt` and issues one opaque process-local lease under the same Rust lock; the lease captures that file's SHA-256 and every save rechecks it. Saves also require the immutable lease, matching checkpoint fingerprint, and a higher revision. `analysis.json.checkpoint.nextCueOffset` records consumed **input cues** (never inferred from output count), and 50 cues remain the canonical transaction size. Every validated partial-batch cue is first atomically saved to local-only `analysis.inflight.json` (maximum 50 entries, 256 KiB per entry, 8 MiB file) before it appears in the UI, so visible preview means disk-persisted. Canonical `analysis.json` is committed before inflight cleanup; process death between those operations is safe because restart rejects a journal already covered by the canonical offset. Foreground → background → foreground transfers the same session/lease and journal. Killing whatSub loses only provider output that had not yet become a validated UI preview; restart adopts the matching journal and requests only the missing cues in that batch. Summary is not partially journaled and restarts as one request. Reset, retranscription, delete, and cloud materialization must invalidate artifacts through the Rust analysis-store destructive boundary; successful materialization removes the old journal only after transcript and canonical analysis are both published, while failed replacement preserves the matching old journal.
- **Managed Pro quota exhaustion is a recoverable checkpoint stop, not an upsell.** Relay `quota_exceeded` errors retain `used`, `limit`, and `periodResetAt`; foreground and background jobs preserve the durable `nextCueOffset`, show the Beijing reset time, suppress the misleading “升级 Pro” CTA, and only enable “继续解析” after a known reset time has passed. “切换自己的 API” deep-links to `/settings?highlight=llm-provider`. Import performs a best-effort quota preflight only for `vendorId === "whatsub-managed"` so an already-exhausted Pro bucket does not waste time downloading or transcribing; BYOK skips the request entirely and quota-check network failures fail open.
- **Destructive boundaries are Rust-authoritative.** Delete and import cleanup hold the lease coordinator lock from revocation through filesystem removal, so a replacement producer cannot start in the middle. Cloud materialization stages transcript + analysis together and publishes `analysis.json` last under the same lock; rollback restores old analysis only after the old transcript is confirmed restored, otherwise analysis stays hidden. `cancel_import` returns success only after the matching child process emits `Terminated` and cleanup finishes; timeout or event-channel closure keeps the job fenced. A filesystem-only cleanup retry holds the exact job-id registry lock through cleanup and unregister, so duplicate cancel callers cannot touch a replacement. Legacy `analysis.generation.json` / `.tmp` / `.bak` artifacts are reconciled once at load and then removed—there is no steady-state generation sidecar. The remote import queue is still not restart-safe because its server contract has no desktop video id, stage, or checkpoint fields.
...
### Existing-video overwrite

Video identity is deterministic for both URLs and local files, so every interactive import first calls Rust `import_preflight` and receives `missing`, `existing`, or `running`. `running` never offers replacement; the same-video registry remains the final concurrency fence. `existing` opens the app-styled “视频已存在” confirmation, and only an explicit confirmation sends `ImportRequest.overwrite=true`. The field defaults to false, so AI tools, remote queue workers, and older frontends cannot overwrite silently; Rust independently rejects an existing entry with `video_exists:<id>` when authorization is absent.

Confirmed replacement is copy-on-success: the complete yt-dlp/local conversion + Whisper pipeline runs in a unique sibling staging directory while the old entry and files remain usable. Only after transcription succeeds does Rust enter the analysis destructive boundary, rename the canonical directory to a backup, promote staging, write canonical `videoDir`/thumbnail paths, clear stale sync metadata, and remove the backup. Promotion/index failure rolls the old directory back; cancellation or pipeline failure deletes staging only. Never change replacement cleanup to call `library_delete`, and never persist staging paths in `LibraryEntry` or the returned `srtPath`.
