...
- **Analysis persistence = runtime lease + exact input checkpoint.** Rust issues one opaque, process-local lease per video producer; every save carries that immutable lease and is accepted only for the same transcript fingerprint with a higher revision. `analysis.json.checkpoint.nextCueOffset` records consumed **input cues** (never inferred from output count), and Rust atomically replaces the single visible `analysis.json` before Zustand/UI publishes it. Foreground → background → foreground transfers the same session/lease. Killing whatSub loses at most the current uncommitted DeepSeek batch; restart opens a fresh lease and repeats only that batch. If `transcript.srt` is already valid, continuation never reruns Whisper; an interrupted summary retries summary only.
- **Destructive boundaries are Rust-authoritative.** Delete, reset, cloud replacement, and import cancellation revoke the active lease before filesystem cleanup, so late output cannot resurrect deleted/reimported analysis. `cancel_import` returns success only after the matching child process exits and cleanup finishes; the foreground modal and background queue remain visible until that confirmation. Legacy `analysis.generation.json` / `.tmp` / `.bak` artifacts are reconciled once at load and then removed—there is no steady-state generation sidecar. The remote import queue is still not restart-safe because its server contract has no desktop video id, stage, or checkpoint fields.
