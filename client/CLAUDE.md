...
- **Analysis persistence = runtime lease + exact input checkpoint.** `openStoredAnalysisSession` reads `transcript.srt` and issues one opaque process-local lease under the same Rust lock; the lease captures that file's SHA-256 and every save rechecks it. Saves also require the immutable lease, matching checkpoint fingerprint, and a higher revision. `analysis.json.checkpoint.nextCueOffset` records consumed **input cues** (never inferred from output count), and each 50-cue batch advances it only after every cue is resolved and Rust atomically replaces `analysis.json`. Foreground and background may display uncommitted previews, but takeover/restart always use the durable session snapshot; cancel, terminal failure, or stale producer callbacks clear or ignore previews. Foreground → background → foreground transfers the same session/lease. Killing whatSub loses at most the current uncommitted DeepSeek batch; restart opens a fresh lease and repeats only that batch. If `transcript.srt` is already valid, continuation never reruns Whisper; an interrupted summary retries summary only.
