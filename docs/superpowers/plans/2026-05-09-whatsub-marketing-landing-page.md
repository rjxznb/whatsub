...
> **Status (2026-05-09): EXECUTED + LIVE + EXTRACTED.** All 5 tasks (PB1-PB5) shipped. Site at `https://whatsub.eversay.cc/` renders 8-section Next.js page (FinalCTA dropped per later user request). The codebase has been **moved to a standalone public repo at [`github.com/rjxznb/whatsub-website`](https://github.com/rjxznb/whatsub-website)** — references in this plan to `client-website/` paths in this repo are historical. Future website work happens in the standalone repo. Project doc with current architecture: `whatsub-website/CLAUDE.md` in the new repo.
