...
4. **`quota_exceeded` 是静默 drop 的例外**:语料库错误默认静默(插件 spec §13),但"配额满"必须**显式提示 + 升级入口**——否则用户一直存、啥也没存还以为成功(撞上当前"乐观显示✓已收藏"的毛病)。
5. **`quota_exceeded` 不进重试队列**:它是确定性错误(跟 blocklist 一样),队列只重试 `network` / `rate_limited`。
...
| D6 | 撞限 → `403`,带 `used` + `limit`。**corpus 走 `{ok:false,reason:"quota_exceeded"}`、library 走 `{error:"quota_exceeded"}`**(各自沿用现有 envelope) | 客户端按 endpoint 区分是哪个配额,无需 quota 字段 |
...
- **档**:免费 **50** / 会员 **1000**。
- **拦截**(新建):`POST /api/corpus/contribute` 入口处,`cap = isMember ? 1000 : 50`,`SELECT count(*) WHERE contributor_id = sha256(email)`;`>= cap` → `403 { ok:false, reason:"quota_exceeded", used, limit }`。
...
- **档**:免费 **3** / 会员 **50**。
- **拦截**(**已存在**,本次仅修会员判定):`POST /api/library/sync` 已有 `quotaLimitFor`,但会员只认 `iosSubActive` → 改为 `isMember`。`cap = isMember ? 50 : 3`,按 `videoKey` 计数(仅 OSS 视频条目);`>= cap` → `403 { error:"quota_exceeded", used, limit }`。
...
- 其它确定性错误(含 **`quota_exceeded`**) → **不进队列**(重试无用)。

### 5.3 与 `quota_exceeded` 的交叉(关键)

`quota_exceeded` 既**不重试**,又**不能静默**。所以:
- 队列侧:`enqueueOrSend` 已天然不 enqueue 非 `network/rate_limited` 的 reason ✅,无需特判。
- UI 侧:气泡当前**不读 `save-vocab-result`**(乐观显示)。需特判:**收到 `save-vocab-result` 且 `reason === "quota_exceeded"` 时,不显示"✓ 已收藏",改弹升级提示**(§6)。其它失败仍保持乐观/静默(已进队列会自动补传)。
...
会员到期 / 退款后,若已存超免费档(如语料库 300 条):**保留数据**,只在新增时返回 `quota_exceeded`,提示"已超免费上限,删至 50 以下或续费可继续新增"。Library 同理。

### 6.4 撞限触点
- 插件:`SelectionBubble.save()` 收到 `quota_exceeded` → 二级界面换成升级卡。
- 桌面:`SyncButton` / `library_sync_to_cloud` 收到 `quota_exceeded` → 升级提示。
- iOS:同步发起处拦 + IAP 升级页。
...
3. `routes/corpus.ts` contribute:`cap = isMember ? 1000 : 50` → count → 超则 `403 {ok:false,reason:"quota_exceeded",used,limit}`。curator(`whatsub-curator`)路径**豁免**(种子脚本不受限)。
4. `routes/library.ts`:**改** `quotaLimitFor` 用 `isMember`(原只看 `iosSubActive`);`/sync`、`/import-queue`、`/quota` 三处传 `hasLicense`。错误沿用现有 `{error:"quota_exceeded",used,limit}`。
...
- `SelectionBubble`:读 `save-vocab-result`,`quota_exceeded` → 升级卡(§6.1),带支付宝入口。
- `shared-types` `ClientReason` 加 `quota_exceeded`;`save-vocab-result` 透传 `used/limit`。

### 8.2 桌面(`Get_Video/client`)
- `library_sync_to_cloud` / `SyncButton`:处理 `quota_exceeded` → 升级提示(支付宝)。

### 8.3 iOS(独立仓,契约)
- 同步/保存撞 `quota_exceeded` → IAP 升级页(**无站外链接**)。
...
- `quota_exceeded`:非静默(§0.4)、非重试(§0.5)。
...
- **插件**:`save-vocab` 走队列(成功/transient 入队/`quota_exceeded` 不入队);`SelectionBubble` 收到 `quota_exceeded` 显示升级卡而非"✓ 已收藏"。
- **桌面**:`library_sync` 撞 `quota_exceeded` 的 UI。

---

## 11. 落地顺序

1. **后端先行**(`whatsub-license`):`hasCorpusAccess` 补订阅 + 两个 count 拦截 + `quota_exceeded` + config + 测试 → 部署。这是真源,先稳。
2. **插件**:可靠队列 + `quota_exceeded` 升级卡。
3. **桌面**:Library 撞限提示。
